...
TMP_MICROSTRUCTURE = ['Simulating no microstructure', 'Simulating microstructure']
DIR_ORIENTATIONS = '12_orientations'
MU_FOLDER = ['Phantom_no_microstructure', 'Phantom_microstructure']
PHASE_FILES = 'bulk_phase.nii.gz'
CHI_FILE = 'chi.nii.gz'
ANGLES_FILES = 'angles_2.mat'
N_ORIENTATIONS = 12
...
def main():
    for n_mode in MODEL_TENSOR:
        actual_model = os.path.join(IMG_ROOT, 'Simulated_data', n_mode, DIR_ORIENTATIONS)
        angles_file = os.path.join(actual_model, ANGLES_FILES)
        print('====================================')
        print(n_mode)
        for n_mu in range(2):
            print(TMP_MICROSTRUCTURE[n_mu])
            actual_folder = os.path.join(actual_model, MU_FOLDER[n_mu])

            phi_file = os.path.join(actual_folder, PHASE_FILES)
            sti_name = os.path.join(actual_folder, CHI_FILE)
            eig_name = os.path.join(actual_folder, EIG_VAL_NAME)
            v1_name = os.path.join(actual_folder, EIG_V1_NAME)
            v2_name = os.path.join(actual_folder, EIG_V2_NAME)
            v3_name = os.path.join(actual_folder, EIG_V3_NAME)
            ...
            chi = sim_tensor(phase_file=phi_file, angles_file=angles_file,
                             num_orientations=N_ORIENTATIONS, phi_scale=phase_scale, mask=MASK_FILE)
            print('... Saving tensor')
            hdr = nii.header
