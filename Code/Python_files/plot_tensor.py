...
ACTUAL_RECONSTRUCTION = 'STI_suite'
ROOT_DIR = os.path.join('..', '..', 'Imagenes', 'Phantom_real_data')
ROOT_MODEL = os.path.join(ROOT_DIR, 'Phantom_tensor')
RECONSTRUCTION_ROOT = os.path.join(ROOT_DIR, 'Noised_data', ACTUAL_RECONSTRUCTION)
...
ROOT_GT_DIFFUSION = os.path.join(ROOT_MODEL, 'Diffusion_sti')
ROOT_GT_SUSCEPTIBILITY = os.path.join(ROOT_MODEL, 'Susceptibility_sti')

ROOT_ORIENTATIONS = ['6_orientations', '12_orientations']
MASK_NAME = os.path.join(ROOT_DIR, 'Masks', 'brain_mask.nii.gz')

ROOT_PAPER = os.path.join('..', '..', 'Papers', 'STI brain phantom', 'Second_dataset')
ROOT_FIGURES = os.path.join(ROOT_PAPER, 'Phantom_models', 'Noised_images', ACTUAL_RECONSTRUCTION)
# ROOT_FIGURES = os.path.join(ROOT_PAPER, 'Phantom_models', 'Reconstructed_models')

CHI_DTI_FILE = os.path.join(ROOT_MODEL, 'chi_dti_filt.nii.gz')
CHI_STI_FILE = os.path.join(ROOT_MODEL, 'chi_sti_filt.nii.gz')
CHI_DTI_6_FILE = os.path.join(ROOT_DIFFUSION, ROOT_ORIENTATIONS[0], 'chi_6_orientations.nii.gz')
CHI_STI_6_FILE = os.path.join(ROOT_SUSCEPTIBILITY, ROOT_ORIENTATIONS[0], 'chi_6_orientations.nii.gz')
CHI_DTI_12_FILE = os.path.join(ROOT_DIFFUSION, ROOT_ORIENTATIONS[1], 'chi_12_orientations.nii.gz')
CHI_STI_12_FILE = os.path.join(ROOT_SUSCEPTIBILITY, ROOT_ORIENTATIONS[1], 'chi_12_orientations.nii.gz')
...
def show_img(tensor, cut, min_max, path_image):
    fig, axs = plt.subplots(1, 3, figsize=(6.75, 4.75))
    indexes = [0, 3, 5]
    positions = [0.03, 0.33, 0.63]
    legends = [r'$\chi_{11}$', r'$\chi_{22}$', r'$\chi_{33}$']
    tmp_pos = 0
    for m in range(3):
        tmp = [positions[tmp_pos], 0.53, 0.35, 0.43]
        # Start next subplot
        axs[m].imshow(np.fliplr(np.rot90(tensor[:, :, cut, indexes[m]])), cmap='gray', vmin=min_max[0], vmax=min_max[1])
        axs[m].set_position(pos=tmp)
        axs[m].text(20, 30, legends[tmp_pos], color='white', fontsize=16)
        axs[m].set_xticks([])
        axs[m].set_yticks([])
        tmp_pos += 1
    # mappable = plt.cm.ScalarMappable(norm=plt.Normalize(vmin=-0.1, vmax=0.1), cmap='gray')
    ...
    # plt.imshow(np.fliplr(np.rot90(tensor[:, :, cut])), cmap='gray', vmin=min_max[0], vmax=min_max[1])
    # plt.axis('off')
...
def main():
    # Diffusion
    chi_dti = read_img(CHI_DTI_FILE)
    fig_path = os.path.join(ROOT_FIGURES, 'chi_dti_gt.png')
    show_img(chi_dti, cut=46, min_max=[-0.1, 0.1], path_image=fig_path)

    chi_dti_12 = read_img(CHI_DTI_12_FILE)
    fig_path = os.path.join(ROOT_FIGURES, 'chi_dti_12_orientations.png')
    show_img(chi_dti_12, cut=46, min_max=[-0.1, 0.1], path_image=fig_path)

    fig_path = os.path.join(ROOT_FIGURES, 'chi_dti_gt_12.png')
    show_img(chi_dti - chi_dti_12, cut=46, min_max=[-0.07, 0.07], path_image=fig_path)

    chi_dti_6 = read_img(CHI_DTI_6_FILE)
    fig_path = os.path.join(ROOT_FIGURES, 'chi_dti_6_orientations.png')
    show_img(chi_dti_6, cut=46, min_max=[-0.1, 0.1], path_image=fig_path)

    fig_path = os.path.join(ROOT_FIGURES, 'chi_dti_gt_6.png')
    show_img(chi_dti - chi_dti_6, cut=46, min_max=[-0.07, 0.07], path_image=fig_path)

    # Susceptibility
    chi_sti = read_img(CHI_STI_FILE)
    fig_path = os.path.join(ROOT_FIGURES, 'chi_sti_gt.png')
    show_img(chi_sti, cut=46, min_max=[-0.1, 0.1], path_image=fig_path)

    chi_sti_12 = read_img(CHI_STI_12_FILE)
    fig_path = os.path.join(ROOT_FIGURES, 'chi_sti_12_orientations.png')
    show_img(chi_sti_12, cut=46, min_max=[-0.1, 0.1], path_image=fig_path)

    fig_path = os.path.join(ROOT_FIGURES, 'chi_sti_gt_12.png')
    show_img(chi_sti - chi_sti_12, cut=46, min_max=[-0.07, 0.07], path_image=fig_path)

    chi_sti_6 = read_img(CHI_STI_6_FILE)
    fig_path = os.path.join(ROOT_FIGURES, 'chi_sti_6_orientations.png')
    show_img(chi_sti_6, cut=46, min_max=[-0.1, 0.1], path_image=fig_path)

    fig_path = os.path.join(ROOT_FIGURES, 'chi_sti_gt_6.png')
    show_img(chi_sti - chi_sti_6, cut=46, min_max=[-0.07, 0.07], path_image=fig_path)
